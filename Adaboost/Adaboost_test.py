...
def calculate_entropy(groups, labels):
    ...
    for attribute in groups:
        SumTerm = sum([this_elem[-1] for this_elem in groups[attribute]])
        group_weight = group_weight + SumTerm
    entropy = 0.0
    for attribute in groups:
        tot_length = float(len(groups[attribute]))
        if tot_length == 0:
            continue
        prob = 0
        sum1 = sum([this_elem[-1] for this_elem in groups[attribute]])
        for class_val in labels:
            p = sum([this_elem[-1] for this_elem in groups[attribute] if this_elem[-2] == class_val]) / sum1
            if p == 0:
                p_temp = 0
            else:
                p_temp = p * -1 * math.log(p, 2)
            prob += p_temp
        entropy += prob * sum([this_elem[-1] for this_elem in groups[attribute]]) / group_weight
    return entropy
...
empty_weights = np.ones(len(testing_data)) / len(testing_data)
testing_data = store_weights(testing_data, empty_weights)
actual_labels_binary = convert_to_binary([elem[-2] for elem in testing_data])
...
def calculate_error_in_this_iteration(iter):
    ...
    for this_iter in range(1,iter):
        ...
        [prediction_per_stump, vote_per_stump, w] = Adaboost(this_iter,testing_data)
        final_prediction = calculate_final_prediction(prediction_per_stump, vote_per_stump, len(testing_data), this_iter)
